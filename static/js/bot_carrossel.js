const container = document.querySelector(".outer-carrossel");
...
// Initialize variables that will change state.
...
const createcarrossel = () => {
  ...
  const length = carrosselItems.length; // Length of the array
  const degress = 360 / length; // Degrees per item
  const gap = 3; // Space between each item
...
// Function that smoothens the animation
const lerp = (a, b, n) => {
  return n * (a - b) + b;
};
...
const distanceZ = (widthElement, length, gap) => {
  return widthElement / 2 / Math.tan(Math.PI / length) + gap; // Z distance of the items
};

// Calculate the height of the container using the field of view and perspective distance
...
// Calculate the field of view of the carousel
...
// Get the X position and evaluate if the position is right or left
const getPosX = (x) => {
  currentMousePos = x;
...
const onResize = () => {
  // Get the size properties of the carousel
